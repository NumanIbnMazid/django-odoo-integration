...
def set_odoo_client():
    ...
    try:
        settings.odoo = erppeek.Client(f"{settings.ODOO_HOST}:{settings.ODOO_PORT}", db=settings.ODOO_DB,
                                       user=settings.ODOO_USER, password=settings.ODOO_PASSWORD, verbose=False)
